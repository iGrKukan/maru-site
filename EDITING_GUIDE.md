...
- **Как опубликовано:** локальный Python-сервер на `127.0.0.1:8200` (файл `app.py`) проброшен наружу через **ngrok** на домен `business-automation.ngrok.app`. Оба процесса подняты через **launchd** (автозапуск + авто-рестарт, переживают перезагрузку).

> ⚠️ **Важно:** сайт обслуживается ИМЕННО с этого Mac. Чтобы правки «уехали в интернет», файлы надо менять здесь, в `~/presentation-site/`. Никакого git-деплоя или внешнего хостинга нет. Перенос на другую машину — см. раздел 6.
...
Есть два типа правок:

### A) Правка контента сайта (HTML/CSS/тексты в `public/`) — публикуется МГНОВЕННО
Сервер отдаёт файлы из `public/` с диска при каждом запросе (для HTML стоит `Cache-Control: no-cache`). Поэтому:
1. Отредактировал любой файл в `public/`.
2. Обновил страницу в браузере (Cmd+Shift+R для жёсткого обновления). **Всё. Рестарт не нужен.**

### B) Правка логики сервера (`app.py`) — нужен рестарт
```bash
